...
public class PassportApplet extends Applet implements ISO7816 {
    ...
    // static final byte CA_INITIALIZED = 0x40; // DEPRECATED: One-step CA no longer needs this state
    ...
    private static final byte EC_PRIVKEY_TAG = 0x63;
    
    private static final byte EC_PUBKEY_TAG = 0x64;
    ...
    private static final boolean DEBUG_CA_CRASH = false;  // TODO: Set to false in production
    ...
    private void processPutData(APDU apdu) {
        ...
        } else if (p1 == 0 && p2 == EC_PRIVKEY_TAG) {
            // CA private key import handling
            // Use same BER-TLV format as AA key import
            buffer_p = BERTLVScanner.readTag(buffer, buffer_p); // tag == EC_PRIVKEY_TAG
            buffer_p = BERTLVScanner.readLength(buffer, buffer_p); // length == 00
            buffer_p = BERTLVScanner.skipValue();
            buffer_p = BERTLVScanner.readTag(buffer, buffer_p); // tag == 04 (OCTET STRING)
            short ecPrivOffset = BERTLVScanner.readLength(buffer, buffer_p);
            short ecPrivLength = BERTLVScanner.valueLength;

            // Verify length for P-256
            if (ecPrivLength != 32) {
                ISOException.throwIt(SW_WRONG_LENGTH);
            }

            // Copy S value
            Util.arrayCopy(buffer, ecPrivOffset, caPrivateKeyS, (short)0, ecPrivLength);
    ...
    private short processMSE(APDU apdu, boolean protectedApdu) {
        ...
        // Parse the data - expecting terminal public key with tag 0x91
        // Format: [0x91][length][public key data]
        ...
        // Read length
        short keyLen = buffer[offset++];
        if (keyLen == (byte)0x81) {
            keyLen = (short)(buffer[offset++] & 0xFF);
        }
        
        // Verify public key length (65 bytes for P-256 uncompressed)
        if (keyLen != 65) {
            ISOException.throwIt(SW_WRONG_LENGTH);
        }
        ...
        // Clear sensitive data for forward secrecy
     ...
     * Note: CA functionality has been moved to processMSE for one-step implementation
     */
    private short processGeneralAuthenticate(APDU apdu, boolean protectedApdu) {
        // CA is now handled entirely in processMSE (one-step mode)
        // This method no longer handles CA operations
        ISOException.throwIt(SW_INS_NOT_SUPPORTED);
        return (short)0;
    }
     ...
     * file.
     * 
     * FIXED VERSION: Handle SM mode correctly based on Martijn's 2006 design
     * 
     * @param apdu
