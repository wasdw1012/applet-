...
def convert_image_to_jpeg2000_compact(input_path, target_size, min_size, max_size):
    ...
    for photo_size in reversed(PASSPORT_PHOTO_SIZES):
        ...
        for ratio_adjust in [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]:
            temp_path = 'temp_compact.jp2'  # 定义temp_path
...
def create_facial_record_header():
    header = b''
    header += b'FAC\x00'  # 不是 b'FAC '，必须是FAC加NULL字节
    header += b'010\x00'  # 不是 b'0100'，必须是010加NULL字节
...
def create_facial_information(image_type: int, width: int, height: int) -> bytes:
    ...
    image_info += struct.pack('>B', FACE_IMAGE_TYPE_FULL_FRONTAL)  # 新常量
...
def create_biometric_data_block(image_data: bytes, image_type: int, width: int, height: int) -> bytes:
    ...
    facial_header_prefix = b'FAC\x00' + b'010\x00'  # 修复为正确的值
...
def generate_dg2_compact(image_path, output_path, format_preference, min_size, max_size):
    ...
    # 强制使用JPEG2000
    if not HAS_JP2_SUPPORT:
        logging.error("JPEG2000支持是必需的！请安装: pip install glymur")
        return False
    ...
    try:
        ...
        if jp2_signature in image_data[:20]:
            ...
            if image_data.startswith(b'\xFF\xD8'):
                ...
                raise ValueError("未知的图像格式，严格验证失败")
