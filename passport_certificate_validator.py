                    ...
                    for i in range(len(decoded)):
                        try:
                            ...
                            if hasattr(security_info, '__getitem__') and len(security_info) >= 2:
                                # 修复：正确提取OID为点分十进制格式
                                protocol_oid = security_info[0]
                                if hasattr(protocol_oid, 'prettyPrint'):
                                    protocol_oid = protocol_oid.prettyPrint()
                                else:
                                    protocol_oid = str(protocol_oid)
                                ...
                                if protocol_oid in self.SUPPORTED_PROTOCOLS:
                                    ...
                                    if protocol_info['type'] == 'CA' and len(security_info) >= 3:
                                        ...
                                        # }
                                        try:
                                            # 修复：处理pyasn1的Integer对象
                                            version = security_info[1]
                                            if hasattr(version, 'hasValue') and version.hasValue():
                                                version = int(version)
                                            else:
                                            ...
                                            if len(security_info) > 2:
                                                key_id = security_info[2]
                                                if hasattr(key_id, 'hasValue') and key_id.hasValue():
                                                    key_id = int(key_id)
                                                else:
                                                    key_id = int(str(key_id))
