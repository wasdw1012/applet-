...
class PassportCertificateValidator:
    ...
    def validate_csca_certificate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result['details']['not_valid_before'] = cert.not_valid_before_utc.isoformat()
            result['details']['not_valid_after'] = cert.not_valid_after_utc.isoformat()
    ...
    def validate_ca_s_value(self) -> Dict[str, Any]:
        """验证CA私钥S值"""
        self.log("="*50)
        self.log("开始验证CA私钥S值")
        ...
        try:
            # 读取S值
            s_value_data = self.read_file(self.FILE_PATHS['ca_s_value'])
        ...
        self.validation_results['ca_s_value'] = result
        return result
    ...
    def validate_trust_chain_closure(self) -> Dict[str, Any]:
        ...
        # CA S值与DG14协议关联
        if 'ca_s_value' in self.validation_results and 'dg14_protocols' in self.trust_chain:
            ca_protocols = [p for p in self.trust_chain['dg14_protocols'] if p['type'] == 'CA']
            if ca_protocols:
                chain_links.append({
                    'from': 'DG14',
    ...
    def generate_report(self) -> str:
        ...
        # 6. CA S值验证结果
        if 'ca_s_value' in self.validation_results:
            report.append("-" * 80)
            report.append("6. CA私钥S值验证结果")
            report.append("-" * 80)
            ca = self.validation_results['ca_s_value']
            report.append(f"文件路径: {self.FILE_PATHS['ca_s_value']}")
