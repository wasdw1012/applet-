...
class UnifiedCertChainGenerator:
    ...
    def generate_dsc_certificate(self):
        ...
        dsc_subject_attributes = [
            x509.NameAttribute(NameOID.COUNTRY_NAME, self.country_2),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, f"{country_info['adjective']} Government"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "National Immigration Administration of PRC"),
            # 注意：具体签发机构OU先不加进来
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Passport"),
            x509.NameAttribute(NameOID.COMMON_NAME, cn_value),
        ]

        # 只有当 specific_issuing_authority 是一个非空字符串时，才把它插入到正确的位置
        if self.specific_issuing_authority:
            specific_ou = x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, self.specific_issuing_authority)
            # 插入到第3个位置 (索引从0开始)
            dsc_subject_attributes.insert(3, specific_ou)
            print(f"   已添加具体签发机构OU: {self.specific_issuing_authority}")
...
##################    核心修正    ###########################    
    ...
    def generate_dg14_file(self):
        ...
        # 1. 编码 SubjectPublicKeyInfo
        subject_public_key_info = ca_public_key.public_bytes(
            encoding=serialization.Encoding.DER,
            format=serialization.PublicFormat.SubjectPublicKeyInfo
        )
        
        # 2. ChipAuthenticationPublicKeyInfo
        ca_protocol_oid = bytes.fromhex('04007F00070202030204')  # id-CA-ECDH-AES-CBC-CMAC-256
        
        # 关键修改：添加外层SEQUENCE包装
        chip_auth_public_key_info = TLV.encode(0x30,  # SEQUENCE
            bytes([0x06, len(ca_protocol_oid)]) + ca_protocol_oid +     # protocol OID
            subject_public_key_info +                                     # 公钥
            bytes([0x02, 0x01, 0x01])                                    # keyId = 1
        )
        
        # 3. ChipAuthenticationInfo 
        chip_auth_info = TLV.encode(0x30,
            bytes([0x06, len(ca_protocol_oid)]) + ca_protocol_oid +
            bytes([0x02, 0x01, 0x01])  # version 1 (不是2!)
        )
        
        # 4. SecurityInfos SET
        security_infos = TLV.encode(0x31, 
            chip_auth_public_key_info +    # 现在是正确的
            chip_auth_info
        )
...
```
C={self.country_2}
O={self.country_full_name} Government
OU=Ministry of Foreign Affairs
OU={self.country_full_name} Passport CA
CN={self.country_full_name} Passport Country Signing Certificate
```
...
"""
    ...
    def collect_all_parameters(self):
        ...
        try:
            ...
            print(f"  警告: 无效的密钥索引 '{key_index_str}'，将使用0。")
            key_index_int = 0
        ...
        # 步骤 5: 获取DSC特定的签发机构OU
        print(f"\n DSC签发机构设置:")
        print("  请输入DSC证书中代表“具体签发机构”的OU")
        print("  例如: Exit-Entry Administration of Public Security Department of Guangdong Province")
        self.specific_issuing_authority = input("输入具体签发机构OU: ").strip()
